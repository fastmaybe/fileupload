...
public class SFTPUtils  {
     ...
     * @param sftpFileName sftp端文件名
     */
    public boolean uploadNew(String aid, String directory, String lastDir, String sftpFileName, InputStream input) throws SftpException {
        try {
            if (directory != null && !"".equals(directory)) {
                sftp.cd(directory);
            }
            ...
            sftp.chmod(Integer.parseInt("777", 8), lastDir);
            sftp.cd(lastDir);
            SftpATTRS attrsReport = null;
            try {
                attrsReport = sftp.stat("upload/");
            } catch (Exception e) {
